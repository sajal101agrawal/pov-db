...
RANGE_CHECKS = {
    ...
    "symbol_daily_metrics.call_forward_factor_formula": "SELECT COUNT(*) FROM symbol_daily_metrics WHERE call_fwdfct_3060 IS NOT NULL AND ABS(call_fwdfct_3060 - ((call_iv_30 / NULLIF(SQRT((POWER(call_iv_60, 2) * 60 - POWER(call_iv_30, 2) * 30) / 30), 0)) - 1.0)) > 0.0001",
    "symbol_daily_metrics.put_forward_factor_formula": "SELECT COUNT(*) FROM symbol_daily_metrics WHERE put_fwdfct_3060 IS NOT NULL AND ABS(put_fwdfct_3060 - ((put_iv_30 / NULLIF(SQRT((POWER(put_iv_60, 2) * 60 - POWER(put_iv_30, 2) * 30) / 30), 0)) - 1.0)) > 0.0001",
