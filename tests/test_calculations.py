...
def test_implied_vol_does_not_return_upper_bound_cap() -> None:
    iv = implied_volatility_bisection(500.0, 100.0, 100.0, 30 / 365, 0.06, "CE")
    assert iv is None
...
def test_skew_ignores_implausible_iv_legs() -> None:
    chain = [
        {"option_type": "CE", "delta": 0.25, "iv": 0.22},
        {"option_type": "CE", "delta": 0.20, "iv": 5.00},
        {"option_type": "PE", "delta": -0.25, "iv": 0.30},
        {"option_type": "PE", "delta": -0.20, "iv": 5.00},
    ]
    assert math.isclose(compute_skew(chain, 0.20), 0.30 - 0.22)


def test_skew_returns_null_for_implausible_surface_gap() -> None:
    chain = [
        {"option_type": "CE", "delta": 0.25, "iv": 0.20},
        {"option_type": "PE", "delta": -0.25, "iv": 1.50},
    ]
    assert compute_skew(chain, 0.25) is None
